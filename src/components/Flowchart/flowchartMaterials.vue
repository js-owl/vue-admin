<template>
  <div class="flowchart-materials">
    ...
    />
    <material-add-dialog
      :open.sync="isMaterialsOpen"
      :data.sync="materialsData"
      @saveData="saveMaterials"
    />
  </div>
</template>
  ...
  computed: {
    ...
    commentsById() {
      return this.comments.filter(
        (item) => item.comment_target === 'materialToolTitle'
      )
    }
  },
  methods: {
    addComment() {
      ...
      this.commentData = {
        ...
        comment_target: 'materialToolTitle'
      }
    ...
    editMaterials(row) {
      this.activeRow = row
      Object.assign(this.materialsData, row)
      this.isMaterialsOpen = true
    },
    saveMaterials() {
      Object.assign(this.activeRow, this.materialsData)
    }
  }
}
</script>
