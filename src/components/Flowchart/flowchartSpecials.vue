<template>
  <div class="flowchart-mod">
    ...
    />
    <special-add-dialog
      :open.sync="isSpecialsOpen"
      :data.sync="specialsData"
      @saveData="saveSpecials"
    />
  </div>
</template>
  ...
  computed: {
    ...
    commentsById() {
      return this.comments.filter(
        (item) => item.comment_target === 'specialToolTitle'
      )
    }
  },
  methods: {
    addComment() {
      ...
      this.commentData = {
        ...
        comment_target: 'specialToolTitle'
      }
    ...
    editSpecials(row) {
      this.activeRow = row
      Object.assign(this.specialsData, row)
      this.isSpecialsOpen = true
    },
    saveSpecials() {
      Object.assign(this.activeRow, this.specialsData)
    }
  }
}
</script>
