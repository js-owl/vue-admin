<template>
  <div class="app-container upload_list">
    <el-table
    ...
    >
      ...
      >
        <template #header>
          <router-link :to="'/specification/edit'">
        ...
        <template #default="scope">
          <router-link
            :to="'/specification/edit/' + scope.row.specification_id"
          >
            <el-tooltip
              class="box-item"
              effect="light"
...
export default {
  ...
  methods: {
    ...
    async deleteRow(index, row) {
      const res = await request({
        url: '/specification/' + row.specification_id,
        method: 'delete'
      })
      if (res) {
        this.$notify({
...
.upload_list .el-table--border td:first-child .cell {
  padding-left: 0;
}
